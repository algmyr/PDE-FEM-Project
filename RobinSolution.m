function [U] = RobinSolution(p, t, e)
%
% Purpose:  Solves  - u_tt - laplace u = f
%           in a domain  D,  with boundary conditions a u + b n .
%           grad u = g, where  n  is the outer normal to the boundary.
%   
...
%
    ...
    b = 1000*ones(size(p,2),1);  % Robin BC constant. b large -> Neumann
    g = 0;                          % Dirichlet boundary term
    ...
    G = BoundVector(p,e,b.*g);  % Compute the boundary vector
    B = BoundMatrix(p,e,b);     % Compute the boundary vector
    ...
    ph = pdeplot(p, e, t, 'zdata', U(:,1), 'xydata', U(:,1),...
            'mesh','on','colorbar','off','colormap','cool');
    grid on;
    set(gca,'ZLim',[-4 4]);
    view(0,90);
    ...
    for i = 2:iters;
        ...
        if updateGraph == 1 && mod(i,10)==0
            ...
            if strcmp(kkey, 'f')
                f=@(x,T) 0.2*(T<((forceframes+i)*k))/k* ...
                         (((x(1)-pt(1,1)).^2+(x(2)-pt(1,2)).^2)<0.1);
            elseif strcmp(kkey,'d')
                f=@(x,T) 2*(T<((forceframes+i)*k))/k* ...
                         (((x(1)-pt(1,1)).^2+(x(2)-pt(1,2)).^2)<0.1);
            elseif strcmp(kkey,'s')
                f=@(x,T) 20*(T<((forceframes+i)*k))/k* ...
                         (((x(1)-pt(1,1)).^2+(x(2)-pt(1,2)).^2)<0.1);
        ...
        % Do calculations
        rhs = k^2*(F+G) + M*(2*U(:,i)-U(:,i-1)) - k^2*(S+B)*U(:,i);
        %U(:,i+1) = M\b;
        U(:,i+1) = Minv*rhs; % Multiplying with (pseudo)inverse is MUCH faster!

        % Force Dirichlet condition
        %U(e(1,:),i+1) = 0;
    end
end
...
function B = BoundMatrix(p, e, b)
% Purpose:  Calculate the boundary matrix
    n = size(p, 2);
    B = zeros(n, n);

    for bel = 1 : size(e, 2)
         dB = ElemBoundMatrix(p, e, b, bel);
         nn = e(1:2, bel);
         B(nn, nn) = B(nn, nn) + dB;
    end
end

function G = BoundVector(p,e,bg)
% Purpose:  Calculate the boundary vector
    
    n = size(p, 2);                             % Number of points
    G = zeros(n, 1);                            % Initiate vector
    for bel = 1 : size(e, 2)
        dG = ElemBoundVector(p, e, bg, bel); % Calc subvector
        nn = e(1:2, bel);
        G(nn) = G(nn) + dG;                     % Add submatrix
    end
end

function dB = ElemBoundMatrix(p, e, b, bel)
% Purpose:  Compute the contribution to bound matrix B from a triangle
    h=norm(p(:,e(2, bel))-p(:,e(1, bel))); %Length of boundary segment
    b=b(e(1:2, bel));

    dB=zeros(2,2);
    dB(2,2) = ( b(1)/12 + b(2)/4  )*h;
    dB(1,1) = ( b(1)/4  + b(2)/12 )*h;
    dB(1,2) = ( b(1)    + b(2)    )*h/12;
    dB(2,1) = dB(1,2);
end

function dG = ElemBoundVector(p, e, bg_in, bel)
% Purpose:  Compute the contribution to bound vector G from a triangle
    bg=bg_in(e(1:2,bel));
    
    h=norm(p(:,e(2, bel))-p(:,e(1, bel)));
    
    dG = [bg(1)/3+bg(2)/6; bg(2)/3+bg(1)/6]*h;
end

%% Elements for matrix assembly
